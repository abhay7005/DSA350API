...
import com.dsa360.api.constants.JwtConstatnt;
...
public class JwtUtil implements Serializable {
	...
	private Claims getAllClaimsFromToken(String token) {
		return Jwts.parser().setSigningKey(JwtConstatnt.SIGNING_KEY).parseClaimsJws(token).getBody();
	}
	...
	public String generateToken(Authentication authentication) {
		...
		return Jwts.builder().setSubject(authentication.getName()).claim(JwtConstatnt.AUTHORITIES_KEY, authorities)
	...
	public UsernamePasswordAuthenticationToken getAuthentication(final String token, final Authentication existingAuth,
		...
		final Collection<? extends GrantedAuthority> authorities = Arrays
				.stream(claims.get(JwtConstatnt.AUTHORITIES_KEY).toString().split(",")).map(SimpleGrantedAuthority::new)
